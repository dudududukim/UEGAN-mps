...
def prepare_image(image):
    ...
    image = image.unsqueeze(0)      # duhyeonkim updated : batch dim added
    return image
...
def calc_nima(img_path, result_save_path, epoch):
    ...
    model = NIMA()
    # print(model)
    model.load_state_dict(torch.load('./metrics/NIMA/pretrain-model.pth', map_location=torch.device('cpu')))
    ...
    test_imgs = [f for f in os.listdir(img_path)]               # val_fake_exp images( from self.G(val_real_raw) )
    for i, img in enumerate(test_imgs):
        image = Image.open(os.path.join(img_path, img))
        image = prepare_image(image).to(device)
        with torch.no_grad():
            preds = model(image).detach().cpu().numpy()[0]      # duhyoenkim updated : .data to .detach() / (1,10) to (10, )

        for j, e in enumerate(preds, 1):                        # NIMA score 계산 방식 : 1*e1 + 2*e2 + ... + 10*e10 -> 해당 이미지의 점수 구간을 예측해서 weighted sum을 함
            mean += j * e
